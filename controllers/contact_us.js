...
module.exports = {
    ...
    async getAllContactUssByPassenger(req, res, next) {
        try {
            const contactUss = await ContactUs.findAll({ where: { driverId: null }, include: { model: Passenger, attributes: ['id', 'email', 'firstName', 'lastName'] } });
            return res.status(http_status_codes.StatusCodes.OK).json(contactUss);
        }
        catch (err) {
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
    ...
    async getAllContactUssByDriver(req, res, next) {
        try {
            const contactUss = await ContactUs.findAll({ where: { passengerId: null }, include: { model: Driver, attributes: ['id', 'email', 'firstName', 'lastName'] } });
            return res.status(http_status_codes.StatusCodes.OK).json(contactUss);
        }
        catch (err) {
            return res.status(http_status_codes.StatusCodes.INTERNAL_SERVER_ERROR).json({
